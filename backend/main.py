...
async def get_cycle_wave(asset: str = "btc", lookback: int = 2190, projection: int = 180):
    ...
    # red price forecast: the CYCLE'S relative move applied to today's price (no runaway trend)
    def _cyc_log(i):
    ...
    for k in range(0, projection + 1):
        forecast[(n_disp - 1) + k] = round(dprices[-1] * math.exp(_cyc_log(i_now + k) - base_log), 2)
