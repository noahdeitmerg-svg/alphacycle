...
@media(min-width:600px){ .scores-grid { grid-template-columns: repeat(3,1fr); } }
  ...
  <div id="phase-banner" class="phase-banner anim d3">
    ...
    <div class="phase-inner">
      ...
      <div class="phase-meta">
        <div class="phase-eyebrow">Current Market Cycle Phase</div>
      ...
      <div class="phase-score-widget">
        ...
        <div class="phase-score-lbl">Combined</div>
      </div>
    </div>
  </div>
...
async function loadAllData() {
  ...
  if (BACKEND_URL) {
    try {
      const [prices, btcC, ethC, macC, hist, analyzer] = await Promise.all([
...
function updatePhaseBanner() {
  ...
  $('phase-combined').textContent = Math.round(positionScore);
  $('phase-combined').style.color = col;
  const eyebrow = $('phase-eyebrow');
  if(eyebrow) eyebrow.textContent = S.alphaCyclePosition != null ? 'Alpha Cycle Position™' : 'Current Market Cycle Phase';
  const lbl = $('phase-score-lbl');
  if(lbl) lbl.textContent = S.alphaCyclePosition != null ? 'Position' : 'Combined';
}
