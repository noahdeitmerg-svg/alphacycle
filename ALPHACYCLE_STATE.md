...
8. **Engine 6 · Seasonal Pattern** — Seasonax-style chart: the average year for **BTC (12y) and ETH (10y)**, geometric-mean intra-year path indexed to Jan 1 = 100 (log, smoothed), showing *when in the year* each tends to climb or stall. Read cards: BTC strong Oct/weak Mar, ETH strong Jan/weak Sep. Source: `/api/seasonal-pattern` (BTC from full daily cache, ETH daily from CryptoCompare, day-cached). Context, not a trade signal.
