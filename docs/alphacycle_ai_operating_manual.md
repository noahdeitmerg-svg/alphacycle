...
| Snapshot Endpoint | ✅ | `build_snapshot(..., days_since_top=...)`; `main.py` übergibt Wert aus Short-Term-Context |
...
4. **Version / display:** `ARC_FORMULA_VERSION` in `backend/arc_config.py` (aktuell **1.1**); `arc_display_score(..., k=0.0)` default — UI-Display = Raw, kein Stretch ohne explizites `k`
...
2. ~~**`days_since_top` snapshot bug**~~ — Resolved: `build_snapshot` accepts the param; `main.py` passes it.
