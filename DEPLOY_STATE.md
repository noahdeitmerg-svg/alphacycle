...
## Letzter Session-Status (2026-03-04) — Phase-Coherent Decision Engine
- **Phase-coherent decision engine (main.py)**: Position/Allocation/Confidence in get_arc_summary() leiten sich primär aus der **Phase** (analyzer.get_short_term_context) ab, nicht nur aus dem ARC-Score. Phase-Gruppen: BEAR_PHASES (Early/Mid/Late Bear), BULL_PHASES (Early/Mid Bull), ACCUMULATION_PHASES (Accumulation, Deep Accumulation), LATE_BULL_PHASES (Late Bull). Bear → "WAIT — Bear Market", 20–40% bzw. 0–20% Allocation, Low Confidence, expected_range type "bear_wait". Late Bull → REDUCE, 20–40% bzw. 0–20%. Accumulation/Bull nach ARC-Schwellen (ACCUMULATE/BUY/HOLD/REDUCE). Fallback: Phase unbekannt → bestehende ARC-only-Logik. Response-Keys: phase_context, phase_group (bear|bull|accumulation|late_bull|unknown). _get_expected_range(arc, fwd, dd, phase): bei Phase in BEAR_PHASES return {"type": "bear_wait", "label": "Bear Market — No Entry Signal"}. /api/historical-returns Response um phase_group ergänzt (gleiche Phase-Logik). compute_arc_score/compute_btc_score/ARC-Formel/arc_display_score unverändert.
