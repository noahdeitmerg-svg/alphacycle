...
- **Was wurde geaendert:** Chronik-Hinweis eingefuegt; falscher Blur-Gate-Absatz (2026-03-13) durch kanonische Kurzfassung ersetzt; Architecture Lock „ARC display“ auf k=0 + Hero/HR-Raw aktualisiert; nummerierte Fixes 82–86 zu ARC-Display/Phase/High-Low entzerrt.
- **Warum:** Nutzer-Bestaetigung zur Fortfuehrung der Doku-Bereinigung.
- **Status:** pushed to GitHub
...
- **Was wurde geaendert:** Zonen-Logik und Konstanten folgen `arc_config.ARC_ZONES` (lo inklusiv, hi exklusiv; Euphoria bis 100 inkl.). Bruch-ARC (z. B. 29.5) landet konsistent in Deep Value wie im Kanon.
- **Warum:** `<=29` und `(0,29)` wichen von `0 <= x < 30` ab.
- **Status:** committed and pushed to GitHub

## Letzter Session-Status (2026-04-10) — Hero Orbit: ol-* an Zonen-Mitte (CSS)
- **Datei(en):** `index.html` (Desktop `.ol-dv`..`.ol-eu` top/left neu; `@media (max-width:700px)` angepasst; 400px ohne doppelte ol-Regeln, Kommentar auf 700px-Cascade), `DEPLOY_STATE.md`, `.cursor/rules/permanent-fixes.mdc`
- **Was wurde geaendert:** Label-Platzierung ausgerichtet mit Uhrzeiger-Conic-Zonen; `.orbit-ring` bleibt `overflow: visible`.
- **Status:** pushed to GitHub
...
- **Was wurde geaendert:** Haiku-QA nach jeder Generierung; bei FAIL Sonnet-Retry mit Fehler-Feedback bis 3 Versuche; danach letzte Reply **trotzdem** an Telegram mit **FAIL_3x**-Warnung — **kein** Tweet-Skip wegen QA. Telegram: **QA: PASS (attempt n)** oder **FAILED 3x (reasons)**. Ohne **QA_ENABLED**: wie bisher ein **generate_reply** + klassische zwei Telegram-Nachrichten.
- **Warum:** Mehr Durchsatz; manuelle Pruefung bleibt moeglich.
- **Status:** pushed to GitHub
...
**Blur Gates (Free vs. Paid Sections) — durch Chronik-Hinweis ersetzt; Kanonischer Stand:** `applyBlurGates()` in `index.html`: `tier === 'free'` nur `gate-hero` + `gate-historical-returns` (locked nur wenn `effectivePlan === 'anonymous'`). `tier === 'paid'`: `gate-cycle-overview`, `gate-near-term`, `gate-arc-history`, `gate-arc-momentum`, `gate-decision-engine`, `gate-zone-history`, `gate-content-export` (locked wenn nicht paid/trial). `gate-live-prices`: nie locked. Paid-CTA: **$49/mo** + Stripe Checkout (`openUpgradeModal`). *(Der lange Absatz darüber aus 2026-03-13 listete Cycle/ARC/Live faelschlich als „free“-Gates; nicht mehr gueltig.)*

## Frühere Sessions (vor 2026-03-04)
- **ARC High/Low Engine (weekly extrempoint detection)**: scoring.drawdown_score_hl(prices, price_override); compute_arc_score weekly_high/low Overrides nach compute_btc_score wenn vorhanden; backtest_engine mit high/low/score_display; Live main.py weekly_high/low None. Chart ARC aus score_display (`arc_display_score`; Default seit 2026-04-10 **k=0** = raw).
...
- **ARC display (Chronik):** Frueher k=1.5 UI-Stretch; **aktuell** Default k=0 — siehe Chronik-Hinweis und Architecture Lock.
...
**ARC display (UI only) — Kanon (2026-04-10):** `arc_display_score(arc_raw, k=0.0)` **Default** in `scoring.py`: Display = Raw (kein Stretch). Optionaler Parameter `k > 0` nur fuer explizite Spezialfaelle. **Hero-Grosszahl/Orbit:** `arc_display` aus API; **Hero-Zonenlabel:** roher `arc_score` (`phaseOf(arcRaw)`). **HR „YOU ARE HERE“:** roher `arc_score`. Backtest/API koennen `score_display` weiter aus `arc_display_score` liefern (mit k=0 = raw). Zonen-Grenzen: `arc_config.ARC_ZONES` / `get_zone_name` / `phaseOf` (5 Zonen).
...
56. **5-Zonen backtest_engine.py**: ZONES = [(0,30),(30,40),(40,60),(60,70),(70,101)] (hi exkl., aligned mit arc_config), ZONE_NAMES. /api/historical-returns Keys risk_rising, euphoria (display_mode reduce/drawdown).
...
73. **5-Zonen Backend**: main.py get_zone_name(arc_score) liefert Deep Value/Accumulation/Expansion/Risk Rising/Euphoria (Grenzen `<30/<40/<60/<70` wie arc_config); /api/arc-summary enthaelt zone_name. historical_returns.py: 5 Zonen mit halboffenen in_zone-Baendern; zone_name pro Zone; compute_high_risk_drawdown Euphoria-Zone (prev < 70, curr >= 70). backtest_engine.py: ZONES wie arc_config. /api/historical-returns display_mode reduce fuer risk_rising, drawdown fuer euphoria.
...
82. **ARC display (Archiv — Text veraltet):** Siehe Architecture Lock oben und Session 2026-04-10: Default **k=0**; fruehere k=1.5/k=1.2-Beschreibungen in dieser Nummerierung historisch.
83. **Phase-coherent decision engine (Archiv):** Phase-Gruppen und Response-Keys bleiben relevant; **expected_range** nutzt heute Zone-Stats aus hist_returns (kein `bear_wait` mehr). Details: `permanent-fixes.mdc` und aktuelles `main.py`.
...
86. **ARC High/Low Engine:** `drawdown_score_hl`, `compute_arc_score(..., weekly_high, weekly_low)` Overrides im Backtest wenn High/Low vorhanden; Live weiter **weekly_high/low = None**. Chart-ARC-Linie aus `score_display` / `arc_display_score` — **Default k=0** (siehe Session 2026-04-10); alte „k=1.2 optimal“-Formulierung obsolet.
