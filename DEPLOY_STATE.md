...
- ✅ **Phase label + HR zones**: Post-Top phase labels (Accumulation/Deep Accumulation); elevated reduce, extreme drawdown in API and UI.
...
75. **Phase label and Historical Returns zones**: analyzer.py get_short_term_context() Post-Top block: arc>=65 Bear/Risk Off, >=50 Early Bear, >=35 Accumulation, else Deep Accumulation; phase_scenarios includes Accumulation and Deep Accumulation. main.py /api/historical-returns enriches elevated (display_mode reduce, display_label REDUCE — DO NOT BUY) and extreme (display_mode drawdown, avg/max/min_drawdown from CACHE). index.html fetchHistoricalReturns() elevated shows REDUCE + DO NOT BUY (#f97316), extreme shows drawdown from peak or SELL/REDUCE.
